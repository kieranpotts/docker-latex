...
> [!IMPORTANT]
> Treat the token like a password. Do not commit it to version control. If you
> do, regenerate it immediately via your Docker Hub account settings.

Set the below environment variables. Optionally, add these to your shell profile
(`~/.bashrc`, `~/.zshrc`, etc.) to persist them.
...
The image will then be available to pull globally. You can choose to sync
your local image with the latest one available from Docker Hub, or pin your
